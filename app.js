...
function improveSidebarAccessibility() {
  ...
  els.navToggle.setAttribute('aria-expanded', els.sidebar.classList.contains('open'));
}

// Make sidebar focusable by keyboard:
els.navToggle.addEventListener('click', ()=>{
  els.sidebar.classList.toggle('open');
  els.navToggle.setAttribute('aria-expanded', els.sidebar.classList.contains('open'));
  if (els.sidebar.classList.contains('open')){
    setTimeout(()=>{els.sidebar.focus();}, 100);
  }
});
...
function wire(){
  ...
  window.addEventListener('keydown', (e)=>{
    ...
    if(e.key==='s' ){ e.preventDefault(); els.sidebar.classList.toggle('open'); return; }
  });
}
